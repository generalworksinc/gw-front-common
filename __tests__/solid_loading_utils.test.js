import { eventWithLoading, awaitLoadingWith } from "../solid/features/loading/utils.ts";

describe("solid/features/loading/utils", () => {
  const makeStore = () => {
    const store = {
      isLoading: false,
      startLoading: () => { store.isLoading = true },
      stopLoading: () => { store.isLoading = false },
    };
    return store;
  };
  ...
  test("handles thrown error and ensures stopLoading", async () => {
    const store = makeStore();
    await expect(eventWithLoading(store, () => { throw new Error("boom") })).rejects.toThrow("boom");
    expect(store.isLoading).toBe(false);
  });
  ...
  test("handles async rejected promise and ensures stopLoading", async () => {
    const store = makeStore();
    try {
      await eventWithLoading(store, async () => Promise.reject(new Error("rej")));
